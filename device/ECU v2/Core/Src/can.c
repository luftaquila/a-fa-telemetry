...
int32_t CAN_SETUP(void) {
  ...
  if (ret != HAL_OK) {
    ...
    #ifdef DEBUG_MODE
      printf("[%8lu] [ERR] CAN filter config failed: %ld\r\n", HAL_GetTick(), ret);
    #endif
    return -1;
  }
  ...
  if (ret != HAL_OK) {
    ...
    #ifdef DEBUG_MODE
      printf("[%8lu] [ERR] CAN start failed: %ld\r\n", HAL_GetTick(), ret);
    #endif
    return -2;
  }
  ...
  if (ret != HAL_OK) {
    ...
    #ifdef DEBUG_MODE
      printf("[%8lu] [ERR] CAN RX notification activate failed: %ld\r\n", HAL_GetTick(), ret);
    #endif
    return -3;
  }
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan) {
  ...
  if (ret != HAL_OK) {
    ...
    #ifdef DEBUG_MODE
      printf("[%8lu] [ERR] CAN RX failed: %ld\r\n", HAL_GetTick(), ret);
    #endif
  }
